...
import { studyMagic } from '../magic/magicService.js'
...
export async function gameRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/game/inventory', async (request, reply) => {
    const char = await getActiveCharacter((request.user as { playerId: string }).playerId)
    if (!char) return reply.status(404).send({ success: false })
    const items = await sql<{ id: string; name: string; category: string; quantity: number; durability: number | null }[]>`
      SELECT i.id, it.name, it.category, i.quantity, i.durability
